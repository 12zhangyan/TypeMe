 ...
 *
 * `typeme-jung48-zh-v3`、`typeme-jung48-zh-v4` 与 v2 的题目**逐字相同**，差别只在它们声明的
 * `scoringVersion`（见 `JungPackageLoader.CURRENT_PACKAGE_ID` 的注释）；v4 是当前默认包，
 * 新草稿一律绑定它，所以这里必须能查到说明，否则主流程 48 主测题 + 16 补充题都不会显示
 * "想一个这样的场景"。
 *
 * 复用是**显式且可校验**的，不靠版本号猜：
 * 1. 只有本版本**一条记录都没有**时才会走到这张表（见 `questionExample`），
 *    以后给 v3/v4 补了自己的记录，它们自动优先，不会被这里顶掉；
 * 2. 命中之后仍然逐字比对题面（`scenario`/`textLeft`/`textRight` 或 `statement`），
 *    有一字不同就返回 `null`，绝不会拿旧说法解释一道改过的题；
 * 3. `readingCompanion.spec.ts` 钉住前提"v3/v4 题面 == v2 题面"，题面一改就红，
 *    提醒要么补 v3/v4 自己的记录，要么重新核对后再声明复用。
 *
 * v4 只是把 v3 的阈值分母从 10 改成 5，题目一个字都没动 —— 与 v3 复用 v2 的理由完全相同。
 */
const REUSED_WORDING: Record<string, string> = {
  'typeme-jung48-zh-v3': 'typeme-jung48-zh-v2',
  'typeme-jung48-zh-v4': 'typeme-jung48-zh-v2',
}

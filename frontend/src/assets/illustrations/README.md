...
3. **把地址写进数据库**（2026-09-20 起地址由库表决定，不再有构建期域名开关）：
   - 生成迁移：`node scripts/gen-image-publish.mjs --emit-sql --out backend/src/main/resources/db/migration/V<下一个版本>__illustration_asset.sql`
     （已有文件拒绝覆盖：迁移是历史，改地址请用下面的接口或新建下一个版本号）；
   - 迁移随部署由 Flyway 执行，**建表与种子都是幂等的**（`CREATE TABLE IF NOT EXISTS` + `INSERT IGNORE`），
     所以"某台机器上已经人工建过表"的库重复执行也不会失败；

...
| AI 解读 | 可选、默认关闭；v5 提供有依据的发现、场景核对与一次行动，旧分析仍可读 | 提示词 v3/v4/v5 支持；若管理员仍固定在 v1/v2，界面说明暂不支持 |
| 内容状态 | 内测待审校 | 内测待审校（**尚未经过真人试读与试测**） |

> **两项测评的测试通过都不等于信度或效度验证**：题面与解释仍是 `draft_review_pending`，
> 没有任何真人试读、试测或常模数据。

2026-09-23 AI 分析改造：默认提示词升级为 `typeme-ai-prompt-v5`，新增 `analysis-guided-v3` 输出，继续使用维度摘要发送范围。页面展示依据、场景与核对问题，生成失败保留填写内容和旧分析；历史分析在 AI 关闭时仍可阅读。显式配置的旧版本与已绑定旧任务保持原行为。方案、兼容说明与验证见 [AI 分析方案与验收](docs/2026-09-23-ai-analysis/方案与验收.md)。

2026-09-21 AI 提示词增强：默认版本升级为 `typeme-ai-prompt-v4`，沿用 v3 的维度摘要发送范围与 `analysis-readable-v2` 输出契约。已绑定的旧任务保留原版本；管理员或环境变量显式固定旧版本时不会自动覆盖。变更与验证见 [提示词 v4 说明](docs/optimization/verification/2026-09-21-ai-prompt-v4/REPORT.md)。

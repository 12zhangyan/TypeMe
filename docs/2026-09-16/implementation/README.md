...
### 3.2 阈值（v1.1 冻结；**2026-09-18 起新版走 v3 口径**）

> **现行版本是 v3**：`typeme-jung48-zh-v3` / `scoringVersion = typeme-jung48-score-v3`，
> `B(n) = T(n) = floor(2n/10)`，且最终边界要求 `n > 0`。触发条件**未变**（没人多答题）。
> 下面两行描述的是 v1/v2 的**冻结行为**（旧包、旧草稿、旧报告继续按它解释），不是新草稿的口径。
> 决定记录见契约 §4.1；两版差异只有 `|S| = T(n)` 这一格与 `n=9` 一档。

- （v1/v2）触发补充题：`|S| <= T(n)`，`T(n) = floor(2n/10)`（**v3 同**）
- （v1/v2）算作"倾向较轻"：`|S| <= B(n)`，`B(n) = max(0, T(n) − 1)`（v3：`B(n) = T(n)`）

（v1/v2）`B(9) = 0` 的后果是：**n=9 时只有完全平分（`S = 0`）才算落在边界内**，
任何非零倾向都**不**算"倾向较轻"，会被讲成方向明确。这句话此前写反过（写成"任何非零倾向都算边界"），
2026-09-18 一并订正；v3 下 `B(9) = 1`，`|S| = 1` 这一格改为"倾向较轻"。
